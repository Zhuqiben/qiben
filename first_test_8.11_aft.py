        ...
        if step == 0:
            ...
            while step == 0:
                ...
                if camera_flag == 0:
                    ...
                    Board.setPWMServoPulse(1, 1250, 500)  # NO3机器人，初始云台角度（1号代表上下，2号代表左右）
                    Board.setPWMServoPulse(2, 1450, 500)
                ...
                if not m_x_b_range[0] < mine_x_base < m_x_b_range[1]:
                    ...
                    if mine_x_range[0] < mine_x < mine_x_range[1] and mine_x > 320:
                        ...
                        if abs(mine_x_self) < mine_x_switch:
                            ...
                            SSR.running_action_group('left_move_fast', 2)
                            print("往左移2步")
                            time.sleep(0.2)
...
def baffler():
    ...
    SSR.running_action_group('stand', 1)
    # 放进翻越动作
    SSR.running_action_group('kuayueplus4', 1)
    SSR.running_action_group('stand', 1)
    # 左转身一定的次数
    # turn_flag = False
    turn_flag = False
...
def door():
    ...
    while state == 'door':
        ...
        if step == 0:
            Board.setPWMServoPulse(1, 1250, 500)  # 头部抬高,居中
            Board.setPWMServoPulse(2, 1400, 500)
            time.sleep(0.5)
        ...
        if step == 1:
            ...
            '''
            count = 0
            Board.setPWMServoPulse(1, 980, 500)  # 头部抬头,居中
            Board.setPWMServoPulse(2, 1400, 500)
            time.sleep(0.5)
            while True:
...
def slope_color_detect_top(color, grad=1.0, ksizes=None, defaults=None):
    if ksizes is None:
        ksizes_ = (3, 3)
    else:
        ksizes_ = ksizes
...
def floor_():
    ...
    print('进入floor关卡')
    SSR.running_action_group('up_stair2', 1)
    SSR.running_action_group('232new', 2)
    time.sleep(0.1)
    SSR.running_action_group('up_stair2', 1)
    time.sleep(0.1)
...
def end_door_():
    ...
    end_door_flag = 0
    # 抬头
    Board.setPWMServoPulse(1, 1150, 500)
    Board.setPWMServoPulse(2, 1400, 500)
    time.sleep(0.5)
